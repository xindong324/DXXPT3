...
namespace Telnet
{
    ...
    public partial class frmTelnet : Form
    {
        ...
        {
            //定义charArea
            this.chartAtt1.ChartAreas.Clear();
            ChartArea chRoll = new ChartArea("Roll");
            ChartArea chPitch = new ChartArea("Pitch");
            ChartArea chYaw = new ChartArea("Yaw");
            this.chartAtt1.ChartAreas.Add(chRoll);
            this.chartAtt1.ChartAreas.Add(chPitch);
            this.chartAtt1.ChartAreas.Add(chYaw);


            ///定义存储数据的容器
            this.chartAtt1.Series.Clear();
            Series sRoll = new Series("sRoll");
            Series sPitch = new Series("sPitch");
            Series sYaw = new Series("sYaw");
            sRoll.ChartArea = "Roll";
            sPitch.ChartArea = "Pitch";
            sYaw.ChartArea = "Yaw";
            this.chartAtt1.Series.Add(sRoll);
            this.chartAtt1.Series.Add(sPitch);
            this.chartAtt1.Series.Add(sYaw);


            //设置图标显示样式
            for (int i = 0; i < 3; i++ )
            {
                this.chartAtt1.ChartAreas[i].AxisY.Minimum = -180.0;
                this.chartAtt1.ChartAreas[i].AxisY.Maximum = 180.0;
                this.chartAtt1.ChartAreas[i].AxisX.Interval = 5;
                this.chartAtt1.ChartAreas[i].AxisX.MajorGrid.LineColor = System.Drawing.Color.Silver;
                this.chartAtt1.ChartAreas[i].AxisY.MajorGrid.LineColor = System.Drawing.Color.Black;
                //this.chartAtt1.ChartAreas[i].ShadowColor = System.Drawing.Color.Blue;
                // this.chartAtt1.Titles[i].Text = "XXX显示";
                //this.chartAtt1.Titles[i].ForeColor = Color.RoyalBlue;
               // this.chartAtt1.Titles[i].Font = new System.Drawing.Font("Microsoft Sans Serif", 12F);
                //设置线型
                this.chartAtt1.Series[i].ChartType = SeriesChartType.Line;
                this.chartAtt1.Series[i].Points.Clear();
            }
        ...
        private void PraseBuff(string[] buf)
        {
            //float[] fatt = { 0f, 0f, 0f, 0f, 0f, 0f }; 
            int i = 0, n = buf.Length;
            string[] satt = { "", "", "", "", "", "", };
            byte[] Batt = new byte[24];
            //bool isVal = false;
            //int i = 0;
            
            //string[] hope={"FE","00","C8","1E"} 
            ...
            for (i = 0; i < n - 34; i++)
            {
                ...
                //isVal = false;
                ///////////////////////////////////////////////////////////////////////
                if (buf[i] == "FE" && buf[i + 4] == "C8" && buf[i + 5] == "1E")
                {
                    //isVal = true;
                    // buf[i+]读取姿态数据
                    int k = 0, j = 0, m = 0;
                    for (j = 0; j < 6; j++)
                    {
                        for (k = 0; k < 4; k++)
                        {
                            satt[j] += buf[i + 4 * j + k + 10];

                        }
                        uint num = uint.Parse(satt[j], System.Globalization.NumberStyles.AllowHexSpecifier);
                        byte[] floatVals = BitConverter.GetBytes(num);
                        att[j] = BitConverter.ToSingle(floatVals, 0);
                    }
                    ...
                    for (m = 0; m < 24; m++)
                    {
                        Batt[m] = Convert.ToByte(buf[i + 10 + m], 16);
                    }
                    for (j = 0; j < 6; j++)
                    {
                        att[j] = BitConverter.ToSingle(Batt, 0 + 4 * j);
                    }
                    ...
                    for (ii = 0; ii < 6; ii++)
                    {
                        att[ii] = att[ii] / 3.14159f * 180;
                        if(ii<3)
                        {
                            chartAtt1.Series[ii].Points.Add(att[ii]);
                        }
                        fatt += att[ii].ToString() + "     ";
                    }
                    //Console.WriteLine(fatt);
                    //this.rtbConsole.Text = fatt;
                    this.rtbAtt.Text = fatt;
                   // break;
                }
                i = i + 10 + 24;
                ////////////////////////////////////////////////////////////////////////////////
        ...
        private void SetAtt(string[] hxBuf)
        {
            int i = 0;
            for (i = 0; i < 6; i++)
            {
        ...
        private void ChartClear_Click(object sender, EventArgs e)
        {
            //清空数据点
            //this.chartAtt1.ChartAreas.Clear()
            this.chartAtt1.Series[0].Points.Clear();
            this.chartAtt1.Series[1].Points.Clear();
            this.chartAtt1.Series[2].Points.Clear();
        }

        private void rtbAtt_TextChanged(object sender, EventArgs e)
        {
